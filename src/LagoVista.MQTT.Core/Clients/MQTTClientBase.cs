...
namespace LagoVista.MQTT.Core.Clients
{
    public abstract class MQTTClientBase : IMQTTClientBase
    {
        ...
        public Task<UInt16> SubscribeAsync(MQTTSubscription subscription)
        {
            return SubscribeAsync(new List<MQTTSubscription>() { subscription } );
        }

        public Task<UInt16> SubscribeAsync(List<MQTTSubscription> subscriptions)
        {
            return _mqttClient.SubscribeAsync(subscriptions);
        }
